...
export default function HomeScreen() {
  ...
  const [user, setUser]                   = useState({ username: '', totalScore: 0, avatar: null });
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          setUser({ username: me.username, totalScore: me.totalScore, avatar: me.avatar });
        }
      } catch (e) {
        console.error(e);
      }
      ...
      >
        ...
        <View style={styles.header}>
          <TouchableOpacity onPress={() => navigation.navigate('Profile')}>
            {user.avatar
              ? <Image source={{ uri: fileUrl(user.avatar) }} style={styles.profileImg}/>
              : <Image source={require('../assets/Sign-in.png')} style={styles.profileImg}/>
            }
          </TouchableOpacity>
          <View style={styles.headerTextGroup}>
            <Text style={styles.feedTitle}>Your Feed</Text>
            <Text style={styles.points}>{user.username} · {user.totalScore} pts</Text>
          </View>
        ...
        >
          {filtered.map(post => (
            <View key={post._id} style={styles.feedCard}>
              ...
              {post.imagePath && (
                <Image
                  source={{ uri: fileUrl(post.imagePath) }}
              ...
              <View style={styles.commentBox}>
                <TouchableOpacity
                  onPress={() => pickCommentImage(post._id)}
                ...
                {commentImages[post._id] && (
                  <Image
                    source={{ uri: commentImages[post._id].uri }}
